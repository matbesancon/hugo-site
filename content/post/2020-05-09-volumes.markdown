...
$$V_{0} = \sum\_{k=1}^N V_k(x_k) = \sum\_{k=1}^N \int_0^{x_k} f_k(x) dx$$
...
$$ F(x) = \sum\_{k=1}^N \int_0^{x_k} x f_k(x)dx.$$

To sum it all, the optimization problem finding an equilibrium is:

$$
\\begin{align}
\min_{x} & \sum\_{k=1}^N \int_0^{x_k} x f_k(x)dx \\\\\\\\  
& \text{subject to:} \\\\\\
& G(x) = \sum\_{k=1}^N \int_0^{x_k} f_k(x) dx - V_0 = 0\\\\\\\\
& x_k \geq 0 \,\,\, \forall k
\\end{align}
$$
...
With the vessel shape functions defined above, this looks
roughly like this:
...
the domain defined by $G(x) = 0$ is a manifold defined, a smooth subspace
...
- which improves the objective function as much as possible: $\langle ∇F(x_i), d\rangle$ as low as possible, or equivalently $\langle -∇F(x_i), d\rangle$ maximized;
...
Since convergence seems find around the equilibrium point (the solution does not bounce around it),
